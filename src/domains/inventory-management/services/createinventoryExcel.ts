...
interface Equipment {
  ...
  description: string;
  brand: string;
  serialNumber: string; // Can contain newlines for multiple serials like "SN001\nSN002"
  supplier: string;
  unitCost: number;
  totalCost: number;
...
// Helper function to format date as "Month Day, Year" (e.g., May 31, 2024)
...
export async function createInventoryExcel(
  ...
  // Values array starts with null for column A, then headers for B through N
  ...
  data.forEach((item, index) => {
    ...
    // Values array starts with null for column A
    row.values = [
      null, // Column A (empty)
